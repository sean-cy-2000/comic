...
export class Comic {
    ...
    static async getComic(comic_id) {
        try {
            const result = await psql.query('SELECT * FROM comics WHERE comic_id = $1', [comic_id]);
            return new Comic(result.rows[0].name, result.rows[0].type, result.rows[0].author, result.rows[0].cover, result.rows[0].description);
        }
        catch (err) {
            console.error(`
                錯誤位置: comic.getComic \n
    ...
    static async getChapters(comic_id) {
        const result = await psql.query('SELECT * FROM chapters WHERE comic_id = $1', [comic_id]);
        return result.rows;
    }
}
class chapter {
